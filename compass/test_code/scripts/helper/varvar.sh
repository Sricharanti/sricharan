#!/bin/sh
#This script evaluates compass output and checks the statistical variance of each axis
#This should reveal if the X, Y, and Z axis readings are rotating amongst each other - 
# that occured with one version of the compass driver


export DIGITAL_COMPASS_CODEX=16
export DIGITAL_COMPASS_CODEY=17
export DIGITAL_COMPASS_CODEZ=10
export DIGITAL_COMPASS_TEXTX=Hat0X
export DIGITAL_COMPASS_TEXTY=Hat0Y
export DIGITAL_COMPASS_TEXTZ=Brake
VARMAX=169

#Event: time 222.188385, type 3 (Absolute), code 16 (Hat0X), value 356
#Event: time 222.188385, type 3 (Absolute), code 17 (Hat0Y), value 518
#Event: time 222.188416, type 3 (Absolute), code 10 (Brake), value -71
...
while read line
do
  echo "${line}"|grep -q "type 3 (Absolute), code $DIGITAL_COMPASS_CODEX ($DIGITAL_COMPASS_TEXTX), value"
  HATOXR=`echo $?`
  echo "${line}"|grep -q "type 3 (Absolute), code $DIGITAL_COMPASS_CODEY ($DIGITAL_COMPASS_TEXTY), value"
  HATOYR=`echo $?`
  echo "${line}"|grep -q "type 3 (Absolute), code $DIGITAL_COMPASS_CODEZ ($DIGITAL_COMPASS_TEXTZ), value"
  HATOZR=`echo $?`
