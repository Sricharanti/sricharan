...
export ACCELEROMETER_IRQ_INITIAL=${AMBIENT_LIGHT_DIR_TMP}/acc.irq.initial
export ACCELEROMETER_IRQ_FINAL=${AMBIENT_LIGHT_DIR_TMP}/acc.irq.final
...
if [ `cat $SYSFS_BOARD_REV | grep -c "Tablet"` -ge 1 ]; then
	...
	export ACCELEROMETER_IRQ=`cat /proc/interrupts | grep "mpu6050_accel_irq" | awk '{print $1}' | cut -d: -f1`
else
