...
if [ "$AUDIOLIBRARY" = "ALSA" ]; then
	...
	if [ "$frontend" = "Voice" ]; then
		tinycap $filename -d $port -f 32 -r 16000 -c 2 -du $duration  || exit 1
	else
		tinycap $filename -d $port -f 32 -r 48000 -c 2 -du $duration  || exit 1
	fi
