...
case "$TESTCASEID" in
	...
	10)
                verifyspeed.sh &&  $UTILS_DIR_HANDLERS/handlerCpuAffinity.sh "switch" "sleep 1; i2cset  -r  -y -f  1 0x48  0x2  0x3 b" "5" "15" || exit 1
	;;
	11)
                verifyspeed.sh &&  $UTILS_DIR_HANDLERS/handlerIrqAffinity.sh  "switch" "sleep 1; i2cset  -r  -y -f  1 0x48  0x2  0x3 b" "5" "15" "88" || exit 1
	;;
	12)
